...
# DBTITLE 1,Remove duplicates, keep latest by timestamp
df_filtered = df.withColumn("rowNum", row_number().over(Window.partitionBy('product_id').orderBy(col('date').desc()))).filter(col('rowNum') == 1)
df_filtered.display()
...
df1 = df.groupBy(col('user_id')).agg(sum('actions').alias('total_actions')).orderBy(col('total_actions'), ascending=False).limit(5)
...
df2 = df.withColumn("rowNum", row_number().over(Window.partitionBy("customer_id").orderBy(col("transaction_date").desc()))).filter(col("rowNum") == 1)
display(df2)
